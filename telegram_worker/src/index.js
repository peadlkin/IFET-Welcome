...
function getCorsHeaders(request, env) {
  ...
  // IMPORTANT: Avoid mixing `||` with `?:` to prevent precedence bugs.
  // Rules:
  // - If ALLOWED_ORIGIN is set: only allow that exact origin.
  // - If Origin header is missing: CORS isn't needed, but we can still return allowed (or nothing).
  // - If ALLOWED_ORIGIN is NOT set: allow all origins (reflect origin if present, else "*").
  let allowOrigin = "";
  if (!allowed) {
    allowOrigin = origin || "*";
  } else if (!origin) {
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "OPTIONS") {
      // If origin is not allowed, don't emit ACAO.
    ...
    // Enforce Origin when configured (hard fail for browser requests)
    const origin = request.headers.get("Origin") || "";
    const allowed = String(env.ALLOWED_ORIGIN || "").trim();
